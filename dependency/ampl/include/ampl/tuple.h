...
class BasicTuple {
  ...
  void destroy() {
    if (impl_.size > 0) deleteTuple(impl_);
  }
  ...
  */
  template <bool U>
  BasicTuple(BasicTuple<U> const& other) {
    assign(other.impl_);
  }
  ...
  */
  std::string toString() const {
    if (impl_.size == 0) return "()";
    fmt::MemoryWriter out;
    if (impl_.size == 1) return (*this)[0].toString();
    out << "(" << (*this)[0].toString();
    for (std::size_t i = 1; i < impl_.size; i++) {
      out << ", " << (*this)[i].toString();
    }
    out << ")";
    return out.str();
