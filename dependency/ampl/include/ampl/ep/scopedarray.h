...
of the copy.
*/
AMPLAPI const char* AMPL_CopyString(const char* source, std::size_t size,
                                    ErrorInformation* errorInfo);
...
struct InDLL<const char> {
  static void deletearray(const char* ptr) { AMPL_DeleteString(ptr); }
};
...
class ScopedArray {
  ...
  */
  ...
  template <class Y>
  ScopedArray(ScopedArray<Y>& other) : ptr_(other.release()) {}
  /**
  ...
  */
   ...
   */
  ScopedArray(ScopedArray_ref<T> other) : ptr_(other.yp) {}
